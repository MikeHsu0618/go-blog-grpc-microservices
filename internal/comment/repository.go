...
type Repository interface {
	...
	UpdateByPostIDWithUnscoped(ctx context.Context, postID uint64) error
...
func (r repository) UpdateByPostIDWithUnscoped(ctx context.Context, postID uint64) error {
	return r.db.Unscoped().Model(&Comment{}).Where("post_id = ?", postID).Update("deleted_at", nil).Error
}
